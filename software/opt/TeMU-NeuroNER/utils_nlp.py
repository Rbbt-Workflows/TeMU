...
def load_pretrained_token_embeddings(parameters):
    filepath = parameters['token_pretrained_embedding_filepath']
    picklepath = filepath + '.pickle'

    if path.exists(picklepath):
        token_to_vector = pickle.load(open(picklepath, "rb"))
    else:
        ...
        file_input.close()
        pickle.dump(token_to_vector, open(picklepath, "wb"))
